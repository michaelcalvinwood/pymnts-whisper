...
async function getCleanedTranscript (chunk) {
    ...
    let response = await ai.getTurboResponse(prompt);
...
async function createInitialParagraphs (chunk) {
    const numParagraphs = Math.ceil(chunk.size / 300);
...
async function insertQuotes (chunk) {
    const prompt = `"""Below is a News Article and a related Transcript. Elaborate on the News Article and also incorporate at least three relevent quotes from the Transcript. Be sure to preserve the dynamic and engaging style.
    News Article:
